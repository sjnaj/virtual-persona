...
### event_log 序列化/反序列化
序列化：对每条 `LifeEvent` 调用 `dataclasses.asdict(e)`，存为 JSON 对象数组。
反序列化：`_load_state()` 中对每个 dict 调用 `LifeEvent(**d)` 重建实例。

### 新增方法
- `_load_state()` — **同步方法**，`__init__` 末尾调用。使用 `pathlib.Path.read_text()` 阻塞读文件，在启动阶段事件循环尚未繁忙时调用，可接受。
- `_save_state()` — 每次"有效 tick"末尾调用（`asyncio.create_task` 包装），不阻塞主流程
- `async def _do_save()` — 实际写文件（`Path.write_text` + `json.dumps`）

### "有效 tick"的定义
下列情况下 **不** 触发保存，直接 return：
- `is_sleeping` 为 True 且未达到起床时间（early return）
- 已满足睡眠条件后进入睡眠（emitting `life.sleeping` 后 return）
- `llm.call_json` 返回 `None`（LLM 调用失败）

以下情况触发保存：LLM 成功返回结果并更新了 `current_action` 之后。
...
### 高频行为黑名单
在构建 prompt 前，统计 `event_log` 最近 `min(8, len(event_log))` 条中出现 ≥2 次的 action，将其列入黑名单并注入 prompt：
```
最近这些行为已经出现太多次了，这轮请避免选择：["刷小红书", "刷手机"]
```
若 `event_log` 为空或无重复行为，则不添加此段（冷启动场景自动跳过）。
...
```
使用 `httpx.AsyncClient(timeout=5.0)` 每次调用时创建，保持与 `browser_agent.py` 一致的模式。
取 `current_condition[0]` 中的 `temp_C` 和 `weatherDesc[0].value`，按下表映射为简体中文：

| 英文关键词（contains，忽略大小写） | 中文描述 |
|----------------------------------|---------|
| sunny / clear | 晴 |
...
### 失败兜底
网络请求失败（`httpx.RequestError`、超时）或 JSON 解析异常时，用 `_season_default_weather()` 生成季节默认天气（按月份区间，见下表）并记录 warning，不抛出异常。
...
location 随 mood 联动（讨食→厨房，赖床/发呆→沙发/床上/猫窝，暴走→消失了）。

**切换逻辑**：`LifeSimulator` 维护一个 `_yearago_ticks_since_change: int` 计数器，每次 tick +1。只有当计数器 ≥2（即至少经过 ~30 分钟）且随机数 < 0.35 时才切换状态，切换后重置计数器为 0。这样重启恢复后不会在第一个 tick 就立即改变状态，也避免了每15分钟都频繁抖动。
...
### 持久化
`YearGaoState` 序列化（`dataclasses.asdict()`）进 `life_state.json` 的 `yearago` 字段，`_yearago_ticks_since_change` 也一并保存（键名 `yearago_ticks`），随其他状态一起恢复。反序列化：`YearGaoState(**state["yearago"])`。

### 标识符规范
`YearGaoState` dataclass 名称固定，实例属性名为 `self.yearago`，JSON 键名为 `"yearago"`，全文统一。
...
`get_status()` 返回值扩展，新增字段，保留原有字段不变（`comfort`/`sleep_debt` 不在公开接口中暴露，维持现状）：
```python
{
    "current_action": ...,
    "current_detail": ...,
    "location": ...,
    "is_sleeping": ...,
    "physical": { "energy": ..., "hunger": ... },   # 不变，comfort/sleep_debt 不暴露
    "recent_events": [...],                          # 不变
    "weather": { "condition": "小雨", "temp": 18 }, # 新增
    "yearago": { "location": "沙发", "mood": "发呆" }, # 新增
}
```
...
- `event_log` 为空时黑名单逻辑不报错（冷启动）
- 天气 API 失败（mock `httpx.AsyncClient.get` 抛出 `httpx.RequestError`）时使用季节默认值，不抛异常
- 天气 API 返回未知英文描述时，原文透传，不报错
- 年糕状态按时段更新，`_yearago_ticks_since_change < 2` 时不切换
- 年糕状态持久化后，`YearGaoState(**d)` 反序列化正确
- `event_log` 持久化后，`LifeEvent(**d)` 反序列化正确，`.action` 属性可访问
